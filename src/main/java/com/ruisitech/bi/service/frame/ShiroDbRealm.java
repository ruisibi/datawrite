...
public class ShiroDbRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException {
		...
		User ui = null;
		if(tk.getChkpsd() != null && tk.getChkpsd() == false) {    //sso 登录验证 或者 微信登录 验证
			if (tk.getWxLogin() != null && tk.getWxLogin() == true) {
				ui = userService.getUserByWxid(tk.getUsername());
			} else {
				ui = userService.getUserInfoByToken(tk.getUsername());
			}
			...
			// 验证用户是否可以登录
			if(userInfoByStaffUrl != null && userInfoByStaffUrl.length() > 0) {  //通过sso获取用户信息
				ui = userService.getUserBySSO(userLogin.getStaffId());
				if(ui == null || ui.getId() == null) {
					throw new UnknownAccountException(); // 异常处理，找不到数据
				}
				ui.setPassword(RSBIUtils.getMD5(ui.getPassword().getBytes()));  //md5转换
			}else {
		...
		String tp = "login";  //正常登录
		if(tk.getChkpsd() != null && tk.getChkpsd() == false) {
			if(tk.getWxLogin() != null && tk.getWxLogin() == true) {
				tp = "wx";  //微信登录
			}else if(tk.getJwtLogin() != null && !tk.getJwtLogin()){
				tp = "jwt";
			} else {
				tp = "sso";  //sso登录
			}
