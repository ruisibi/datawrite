  	...
  	<el-dialog fullscreen :append-to-body="true" title="明细数据提取" :visible.sync="show" :close-on-click-modal="false" custom-class="nopadding">
			...
			<el-table :data="tableData" :height="height" border style="width: 100%" header-row-class-name="tableHeadbg">
				<template v-for="(item, index) in cols">
					<el-table-column :key="item.colName" align="center" :prop="'c'+index" :label="item.colNote"></el-table-column>
				</template>
			</el-table>
			<el-pagination
				background
	...
	export default {
		...
		methods: {	
			...
			loadDatas(){
				...
				let comp = this.comp;
				let pms = this.pms;
				var json = {pms:pms, cubeId:comp.cubeId, page:ts.page, rows:ts.rows};
				ajax({
					type: "POST",
