  ...
  <el-form :model="cube" :rules="rules" ref="cubeForm" size="mini">
      <el-tabs v-model="active" type="card">
        <el-tab-pane label="基本信息" name="base">
          ...
          <el-form-item label="对应表单" label-width="100px" prop="tableId">
            <el-select
              v-model="cube.tableId"
              placeholder="请选择"
              @change="selectdSet"
              style=" width:100%;"
              filterable
...
<br />
  ...
  <cubeAddGroup ref="groupForm"></cubeAddGroup>
...
import $ from "jquery"
import cubeAddGroup from './CubeAddGroup'
import expressionAdd from './ExpressAdd'
import dimkpiModify from './DimkpiModify'
...
export default {
  ...
  components: {
		cubeAddGroup, expressionAdd, dimkpiModify
	},
  mounted() {
  ...
  methods: {
      ...
      ajax({
        ...
        success:(resp)=>{
          this.dsetList = resp.rows.map(r=>{
            return {tableId:r.id, dsetName:r.tableNote};
          });
        }
      });
    },
      ...
      this.$refs['cubeForm'].validate(v=>{
        if(v){
         ...
         var cubeKpi = [];
          ...
          let cube = ts.cube;
          let pageJson = {cubeName:cube.name,  desc:cube.desc, tableId:cube.tableId};
          pageJson.dims = cubeDim;
          pageJson.kpis = cubeKpi;
          if(isupdate){
            pageJson.delObj = this.delObj;
            pageJson.id = this.cube.id;
          }
    ...
    selectdSet(cube){
      ajax({
        type:"GET",
        url:"model/listColumns.action",
        data:{tableId:this.cube.tableId},
        success:(resp)=>{
          resp = resp.rows;
          ...
          for(let i=0; i<resp.length; i++){
            let r = resp[i];
            var node = {id:r.colName,text:r.colNote,icon:'glyphicon glyphicon-menu-hamburger',li_attr:{tp:'node', vtype:r.colType, col:r.colName, expression:r.expression}}
            dt.push(node);
          }
    ...
    initRightTree(cube){
      ...
      $("#cuberighttree").jstree({
       ...
       if(cube){  //隐藏用户选择的列
             ...
             cld.forEach(e => {
               let l = rref.get_node(e);
               nodes = nodes.concat(l.id);
             });
    ...
    addgroup(){
      this.$refs['groupForm'].create(false);
    },
    editCalcKpi(){
      this.$refs['expressionForm'].create(false, this.dset);
    },
    editcubecol(){
      ...
      if(node.li_attr.tp == 'kpi' && node.li_attr.calcKpi == 1){
        this.$refs['expressionForm'].create(true, this.dset);
        return;
      }
      if(node.li_attr.tp === 'group'){
        this.$refs['groupForm'].create(true, node);
        return;
      }
      this.$refs['dimKpiForm'].modify(node, this.dset);
    }
  }
};
</script>
