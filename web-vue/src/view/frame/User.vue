		  ...
		  <div class="ibox">
				...
				<div class="btn-group optbtncls" role="group">
				 ...
				 <el-table :data="tableData" @row-click="selectme" border style="width: 100%" header-row-class-name="tableHeadbg">
					...
					<el-table-column align="center" prop="id" label="操作">
						<template slot-scope="scope">
								<a class="btn btn-primary btn-xs" @click.stop="addUserRole(scope.row.id)"> 授权角色 </a>
								<a class="btn btn-primary btn-xs" @click.stop="userMenu(scope.row.id)"> 授权菜单 </a>
						</template>
					</el-table-column>
				</el-table>
				<el-pagination
			...
			<el-dialog title="授权角色" :visible.sync="userRoleDailog">
				  <el-checkbox-group v-model="checkList">
					<el-checkbox v-for="r in roles" :label="r.id" :key="r.id">{{r.roleName}}</el-checkbox>
				  </el-checkbox-group>
			  <div slot="footer" class="dialog-footer">
...
<script>
	...
	export default {
		...
		methods: {
			...
			userMenu:function(id) {
				this.$router.push({name:"userMenu", query: {id}});
			},
