    ...
    <div class="ibox">
      ...
      <div class="ibox-content">
         <div style="margin-bottom:10px;" class="btn-group" role="group">
            <button type="button" class="btn btn-outline btn-default" @click="verifydata(2)">
              <i class="glyphicon glyphicon-ok" style="color:green;"></i> 审核通过
            </button>
            <button type="button" class="btn btn-outline btn-default" @click="verifydata(3)">
              <i class="glyphicon glyphicon-remove" style="color:red;"></i> 审核不通过
            </button>
          </div>
          ...
          <el-table :data="tableData" border style="width: 100%" header-row-class-name="tableHeadbg" @selection-change="handleSelectionChange">>
						<el-table-column label="" width="45" fixed="left" type="selection">
...
export default {
  ...
  methods: {
     ...
     goback(){
      this.$router.push({path:"/verify/TableList", query:{}});
     },
     loadDatas(){
    ...
    verifydata(v){
      ...
      if(confirm("是否确认？")){
        ajax({
          url:"verify/verify.action",
          data:JSON.stringify({tableId: this.json.id, audit:v, ids:this.selects.map(m=>m.tmp_data_id)}),
          type:"POST",
          postJSON:true,
          success:()=>{
            this.loadDatas();
